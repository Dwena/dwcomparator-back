...
public class IndexedPageCustomRepositoryImpl implements IndexedPageCustomRepository {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public void removeOldUnpublishedPages(int days) {
        String req = "SELECT ip FROM IndexedPage ip WHERE ip.published=0 AND ip.creationDate< :d";
        Query query = em.createQuery(req);
        LocalDateTime dateMinusDays = LocalDateTime.now().minusDays(days);
        query.setParameter("d", dateMinusDays);
        List<IndexedPage> lst =  (List<IndexedPage>) query.getResultList();
        for (IndexedPage indexedPage : lst) {
            try {
                em.remove(indexedPage);
            } catch (Exception e) {
                e.printStackTrace();
            } 
        }
        //DELETE FROM indexed_page p WHERE p.published=1 AND DATEDIFF(CURDATE(), p.creation_date)>= 3
